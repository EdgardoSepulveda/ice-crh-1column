...
def cre(datain_path, dataout_path, test, version):
    in_file   = 'output_1Dset_' + test + '_1O_' + version + '.nc' # Just to retrieve number of columns and vertical levels.
    ecrad_out = xr.open_dataset(datain_path + in_file)
    param = ecrad_out.sizes['column']     # Test parameter
    profi = ecrad_out.sizes['half_level'] # Vertical Profile
    ...
    ## Cloud radiative effect at TOA array [W m^-2]:
    cre_sw  = np.zeros((optsh, profi, param))
    cre_lw  = np.zeros((optsh, profi, param))
    cre_net = np.zeros((optsh, profi, param))
    ...
    for opt in range(1, optsh + 1): # per optical scheme
        ...
        for i in range(param):
            ...
            # Cloud Radiative Effect [W m-2]
            cre_lw[opt-1, :, i] = lw_net_flux_cloudy - lw_net_flux_clear
            cre_sw[opt-1, :, i] = sw_net_flux_cloudy - sw_net_flux_clear

        cre_net[opt-1, :, :] = cre_sw[opt-1, :, :] + cre_lw[opt-1, :, :]
...
#         ),
        data_vars = dict(
            cre_sw  = (['scheme', 'level', 'variable'], cre_sw),
            cre_lw  = (['scheme', 'level', 'variable'], cre_lw),
            cre_net = (['scheme', 'level', 'variable'], cre_net),
        ),
        attrs = dict(
            title = 'CRE 1D set from ' + test,
            description = 'SW, LW and Net CRE matrices for ' + test, 
        ),
    )

    out_file = 'CRE_1Dset_' + test + '_' + version + '.nc'
    ds.to_netcdf(dataout_path + out_file)
